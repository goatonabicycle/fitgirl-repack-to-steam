...
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  ...
  const definition = defineContentScript({
    ...
    main() {
      ...
      function getReviewClass(reviewText) {
        return `steam-card-review-${reviewText.toLowerCase().replace(/\s+/g, "-")}`;
      }
      function createSteamCard(result2, gameName) {
        var _a;
        if (result2) {
          const steamLink = document.createElement("a");
          steamLink.href = `https://store.steampowered.com/app/${result2.id}`;
          steamLink.className = "steam-card";
          steamLink.dataset.processing = "true";
          steamLink.target = "_blank";
          const price = ((_a = result2.price) == null ? void 0 : _a.final) ? `$${(result2.price.final / 100).toFixed(2)}` : "Not available";
          const priceHTML = price !== "Not available" ? `<span class="steam-card-separator">•</span>
             <span class="steam-card-price">${price}</span>` : "";
          const reviewText = result2.reviewText || "";
          const reviewHTML = reviewText ? `<span class="steam-card-separator">•</span>
      ...
      async function processElement(element) {
        ...
        const alreadyProcessed = siblings.some(
          (sibling) => {
            var _a2;
            return sibling.classList.contains("steam-card") || sibling.classList.contains("steam-card-loading") || ((_a2 = sibling.dataset) == null ? void 0 : _a2.processing) === "true";
          }
        );
        if (alreadyProcessed) {
          return;
        ...
        (_b = element.parentNode) == null ? void 0 : _b.insertBefore(loadingEl, element.nextSibling);
        const gameName = extractGameName(((_c = element.textContent) == null ? void 0 : _c.trim()) || "");
        try {
          const response = await browser.runtime.sendMessage({ action: "searchGame", gameName });
          const card = createSteamCard((response == null ? void 0 : response.success) ? response.result : null, gameName);
